...
public class CheckUpdateProduct extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException,IOException{
		...
		SaxParser saxparser= new SaxParser(dir+"\\webapps\\Assignment_1_kp\\WEB-INF\\classes\\ProductCatalog.xml");
		...
		if(category.equals("fwatches")){
			...
			for(FitnessWatch fwatch : fwatches){
				if(fwatch.getId().equals(id)){
					...
					pprice=fwatch.getPrice();flag=1;break;
				}
			}
		}
			...
			for(Television television : televisions){
				if(television.getId().equals(id)){
					...
					pprice=television.getPrice();flag=1;break;
				}
			}
		}
