...
function displayTable(responseXML) {
    ...
    if (responseXML != null) 
	{
        var products_collection = responseXML.getElementsByTagName("items")[0];
        if (products_collection.childNodes.length > 0) 
		{
			s_table.setAttribute("border", "1");
            s_table.setAttribute("bordercolor", "black");
			var i;
            for (i = 0; i < products_collection.childNodes.length; i++) 
			{
                var p_instance = products_collection.childNodes[i];
				var p_name = p_instance.getElementsByTagName("itemName")[0];
                var p_id = p_instance.getElementsByTagName("itemId")[0];    
